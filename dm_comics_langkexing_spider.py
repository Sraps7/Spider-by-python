...
def get_chapter(href, comic_name, comic_vol, my_task):
    ...
    total_page = bs(driver.page_source, 'lxml').select('select#page_select option')[-1].text[1:-1]
    ...
    for i in range (1, int(total_page) + 1):
        ...
        # print(src)
        if not os.path.exists("F:\Study\\reading"):
            os.mkdir("F:\Study\\reading")
        if not os.path.exists('F:\Study\\reading/' + comic_name):
            os.mkdir('F:\Study\\reading/' + comic_name)
        if not os.path.exists('F:\Study\\reading/' + comic_name + '/' + comic_vol):
            os.mkdir('F:\Study\\reading/' + comic_name + '/' + comic_vol)
        if not os.path.exists('F:\Study\\reading/' + comic_name + '/' + comic_vol + '/' + str(i) + '.jpg'):
            with open('F:\Study\\reading/' + comic_name + '/' + comic_vol + '/' + str(i) + ".jpg", 'wb') as fs:
...
comic_name = '浪客行'
home_url = 'https://manhua.dmzj.com/lkxlrjk'
